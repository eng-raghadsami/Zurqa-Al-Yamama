...
import { REVIEW_PREVIEW_IMAGE, DECEPTION_TYPES } from "@website/types/mediaVerification";
import { analyzeContentImage } from "@website/services/contentAnalysisService";

export default function ReviewStep() {
  const { form, goPrev, completeSubmission } = useMediaVerification();
  ...
  const fileLabel = form.file?.name ?? "image_upload.jpg";
  ...
  const handleSubmit = async () => {
    ...
    if (!form.file) {
      completeSubmission(null, "يرجى رفع صورة قبل الإرسال.");
      return;
    }

    setLoading(true);
    try {
      const result = await analyzeContentImage(form.file);
      completeSubmission(result);
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "تعذّر تحليل الصورة عبر الخادم";
      completeSubmission(null, message);
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="p-8 md:p-12">
        <div className="text-center mb-10">
          ...
          <p className="font-body-lg text-on-surface-variant max-w-lg mx-auto">
            راجع البيانات أدناه ثم أرسل الصورة لتحليلها عبر خادم زرقاء اليمامة
          </p>
        </div>
            ...
            <div className="aspect-video w-full relative bg-mist-grey overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
                <div className="flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full border border-white/30">
                  ...
                  <span className="text-white text-xs font-label-bold">صورة • {fileLabel}</span>
                </div>
              </div>
            </div>
            <div className="p-6">
              ...
              <p className="font-body-md text-on-surface-variant">
                ستُحلَّل الصورة عبر واجهة{" "}
                <code className="text-xs bg-mist-grey/40 px-1 rounded">/api/images/analyze</code>{" "}
                للكشف عن التلاعب والمحتوى الحساس والعنف والعنصرية.
              </p>
            </div>
          </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  جاري التحليل عبر الخادم...
                </>
              ) : (
                <>
                  <span>تحليل الصورة وإرسال الطلب</span>
